...
\begin{equation}h_t \rightarrow y_t \rightarrow E(y_t) \rightarrow h_{t+1}.\end{equation}
...
\begin{equation}\text{representation} \neq \text{serialization} \neq \text{schedule} \neq \text{modality}.\end{equation}
...
\begin{equation}h_t \rightarrow y_t^{\mathrm{reason}} \rightarrow E(y_t^{\mathrm{reason}}) \rightarrow h_{t+1}.\end{equation}
...
\begin{equation}t_{\mathrm{token}} \neq t_{\mathrm{compute}} \neq t_{\mathrm{world}}.\end{equation}
...
\begin{equation}o^{(m)} = g_m(s) + \epsilon^{(m)},\end{equation}
...
Thus,\begin{equation}\text{one output stream} \neq \text{one internal process}.\end{equation}
This motivates persistent multi-stream latent state. At event $e_n$, maintain
\begin{equation}Z_n=[z_n^{(1)},z_n^{(2)},\ldots,z_n^{(K)}],\end{equation}
...
\begin{equation}a_n=\pi_{\mathrm{route}}(Z_n,e_n), \qquad a_n\in\{0,1\}^K,\end{equation}
\begin{equation}z_{n+1}^{(k)}=\begin{cases}F_k(z_n^{(k)},e_n,Z_n), & a_n^{(k)}=1,\\z_n^{(k)}, & a_n^{(k)}=0.\end{cases}\end{equation}
\section{A Unified Design Principle}
The common principle is to preserve internal state and project only when needed.
\begin{center}\streambox{VISION}\hfill\streambox{AUDIO}\hfill\streambox{TEXT}\hfill\streambox{SENSORS}\vspace{0.35em}
$\downarrow$ modality-specific lenses $\downarrow$\vspace{0.25em}
\fcolorbox{accent}{soft}{\parbox[c][2.2em][c]{0.88\linewidth}{\centering\small\bfseries PERSISTENT LATENT WORLD STATE}}\vspace{0.35em}
\streambox{ENTITY}\hfill\streambox{HYPOTHESIS}\hfill\streambox{PLAN}\hfill\streambox{DIALOGUE}\vspace{0.35em}
$\downarrow$ serialize only when interaction requires it $\downarrow$\vspace{0.25em}
\streambox{SPEECH}\hfill\streambox{ACTION}\hfill\streambox{IMAGE}\hfill\streambox{TOKENS}\end{center}
...
\begin{equation}z_t=P(h_t),\qquad g_t=\sigma(W_g h_t).\end{equation}
Then define the next input embedding as
\begin{equation}x_{t+1}=E(y_t)+\mathbf{1}[y_t=\varnothing]g_t\odot z_t.\label{eq:null}\end{equation}
...
\begin{figure*}[t]\centering\small\textbf{Visible token step}\par\vspace{0.3em}\stepbox{token}\ $\rightarrow$ \stepbox{embedding}\ $\rightarrow$ \stepbox{Transformer}\ $\rightarrow$ \stepbox{LM head}\ $\rightarrow$ \stepbox{visible token}\vspace{0.7em}\textbf{Silent compute step}\par\vspace{0.3em}\stepbox[accent]{$\varnothing$}\ $\rightarrow$ \stepbox{$E(\varnothing)$}\ $+$ \stepbox[accent]{$g_t\odot z_t$}\ $\rightarrow$ \stepbox{Transformer}\ $\rightarrow$ \stepbox[accent]{$\varnothing$ or token}\vspace{0.45em}\fcolorbox{accent}{soft}{\parbox{0.88\textwidth}{\centering\small The recurrence remains in the ordinary autoregressive loop. Compute time can advance while visible token time remains still.}}\caption{Null-token continuous recurrence as one concrete relaxation of token and timing bottlenecks.}\end{figure*}
...
\begin{equation}x_{t+1}=E(y_t)+\sum_{k=1}^{K}g_t^{(k)}\odot P_k(z_t^{(k)}).\label{eq:multi}\end{equation}
...
Text CoT has a special property: computation and a human-readable trace partially share the same channel. Once state can evolve without serialization, this alignment disappears. A richer latent state may improve efficiency or capability while making the internal trajectory less directly legible. Monitorability therefore becomes an explicit objective rather than a free by-product of the interface bottleneck.
Possible tools include auxiliary decoders, probes, sparse verbal checkpoints, causal interventions, latent-state perturbation tests, and consistency checks across modality readouts.
...
\subsection{Discrete bottleneck}Compare explicit CoT with null-token latent carry under matched transformer FLOPs. Remove continuous carry while preserving repeated depth to determine whether gains come from richer feedback or merely extra compute.
\subsection{Temporal bottleneck}Compare fixed recurrent depth with learned variable recurrence. Test whether compute allocation correlates with task difficulty and improves the accuracy--latency frontier.
\subsection{Modality bottleneck}Present the same underlying event through text, image, audio, and combinations thereof. Measure cross-modal consistency of shared state, transfer to non-linguistic action, and degradation when modality-specific intermediate serialization is forced.
\subsection{Seriality bottleneck}Use environments with independently evolving entities and delayed evidence. Compare a single entangled state against explicit persistent components under matched compute. Measure interference, retention, revision cost, and interruption handling.
\subsection{Monitorability cost}As representational freedom increases, measure probe decodability, intervention predictability, sparse-checkpoint fidelity, and whether unsafe internal changes can be detected before output.
...
\section{Discussion}
The success of language models makes it easy to confuse a powerful interface with a native substrate of intelligence. Tokens provide discrete supervision, scalable datasets, and clear communication. Fixed steps simplify optimization and systems engineering. Modality-specific adapters exploit structure. Serial streams make outputs easy to evaluate.
