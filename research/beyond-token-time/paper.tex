...
For a causal decoder, the training interface factorizes a sequence as
\begin{equation}
  p_\theta(x_{1:T}) = \prod_{t=1}^{T} p_\theta(x_t \mid x_{<t}),
\end{equation}
and applies an attention mask $M_{ij}=-\infty$ for $j>i$. A position therefore cannot read subsequent token positions in that forward pass \cite{vaswani2017attention}. This is an externally supplied information-flow constraint over the chosen serialization, not a learned physical causal order among the events represented by those tokens.

The distinction is not that learned weights are order-neutral. Gradients shape them under the causal factorization, positional scheme, and data distribution, so they can encode order-sensitive regularities. The narrower claim is that the mask itself is not stored as a model parameter, and no explicit observer frame or world-event ordering follows from the weights alone.

At inference time, a frame specification $c_{\mathcal F}$ can condition a readout without changing the weights, just as natural-language instructions and demonstrations can condition model behavior without gradient updates \cite{brown2020language,xie2021incontext}:
\begin{equation}
  y_{\mathcal F} = D_\theta\!\left(Z; c_{\mathcal F}\right).
\end{equation}
If $Z$ retains information compatible with several viewpoints, $c_{\mathcal F}$ may select a frame-consistent serialization. A useful but deliberately non-mechanistic metaphor is \emph{holographic restoration}: the prompt acts like a boundary condition for reconstructing one view from distributed state. The academic claim is only \emph{frame-conditioned latent readout}. It does not imply that the weights literally implement a hologram, contain a complete frame-independent world, or will preserve invariants without targeted training and evaluation.

\callout{\textbf{Architectural constraint versus learned consequence.} Causal masking enforces token-position access order. Training under that constraint shapes the weights. A frame prompt then conditions a readout; whether the underlying state supports faithful cross-frame reconstruction is an empirical question.}
...
Causal masking at this stage enforces left-to-right access in each serialized training example. It should not be treated as supervision for a unique physical chronology. Frame labels, causal graphs, or multi-frame renderings must be supplied if the system is expected to separate causal invariants from observer-dependent order.
...
Serialize the same causal event graph under two or more declared frames, then prompt the model with $c_{\mathcal F}$ to reconstruct each view. Hold causal ancestry, entity identity, and outcomes fixed while allowing only the coordinate order of unrelated events to change. Measure invariant accuracy, frame-dependent chronology, and round-trip consistency under $\mathcal F_1 \rightarrow \mathcal F_2 \rightarrow \mathcal F_1$. Ablate the frame prompt, explicit frame supervision, causal mask, and the order of unrelated training events. Failure to preserve invariants would indicate prompt-conditioned narration rather than restoration of a coherent latent event structure.
...
\begin{itemize}
  ...
  \item \textbf{Prompted confabulation:} a plausible frame-specific narrative is generated without a cross-frame-consistent latent event structure.
